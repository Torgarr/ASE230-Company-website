...
function getPageContent($id)
{
    ...
    $data = file_get_contents('../../data/'.$GLOBALS['contents'][$id]);
    return $data;

}

function createPage($name){
    if (file_exists('../../data/'.$name['file_name'].'.html')){
        echo 'File already exists';
    }
    else{
        file_put_contents('../../data/'.$name['file_name'].'.html', $name['file_contents']);
...
function deletePage($name,$index){
    if (!file_exists('../../data/'.$name)){
        ...
        $path = dirname(__DIR__,2).'\\data\\'.$name;
        unlink($path);
